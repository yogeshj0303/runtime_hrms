...
class SalaryVariableController extends Controller
{
    public function index(Request $request)
    {
        ...
        $nonPayableComponents = SalaryComponent::where('not_payable', true)->get();
        $payableComponents = SalaryComponent::where('not_payable', false)->get();
        ...
        if ($request->has('salary_component_id') && $request->has('payroll_month')) {
            ...
            foreach ($employees as $employee) {
                $variables = EmployeeSalaryVariable::where('employee_id', $employee->id)
                ...
                $employee->current_variable_amount = $variables->sum('amount');
                $employee->current_variable_comments = '';
            }
        }

        return view('admin.data_capture.salary_variable', compact(
            'businessUnits', 'locations', 'departments', 'variableComponents', 
            'employees', 'nonPayableComponents', 'payableComponents'
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $variable = EmployeeSalaryVariable::firstOrCreate([
            ...
            $variable->update(['amount' => $totalAmount]);
    ...
    {
        $request->validate([
            'source_component_id' => 'required|exists:salary_components,id',
            'target_component_id' => 'required|exists:salary_components,id',
        ...
        try {
            ...
            $query = EmployeeSalaryVariable::where('salary_component_id', $request->source_component_id)
            ...
            foreach ($sums as $sum) {
                if ($sum->total_amount > 0) {
                    ...
                    EmployeeSalaryVariable::where('employee_id', $sum->employee_id)
                        ->where('salary_component_id', $request->target_component_id)
                        ->where('payroll_month', $targetPayrollMonth)
                        ->delete();

                    EmployeeSalaryVariable::create([
                        'business_id' => $employee->business_id ?? 1,
                        'employee_id' => $sum->employee_id,
                        'salary_component_id' => $request->target_component_id,
    ...
    public function getDetails(Request $request)
    {
        ...
        $parent = EmployeeSalaryVariable::where('employee_id', $request->employee_id)
            ->where('salary_component_id', $request->salary_component_id)
            ->where('payroll_month', $request->payroll_month)
    ...
    {
        $detail = \App\Models\EmployeeSalaryVariableDetail::findOrFail($id);
        $parent = $detail->employeeSalaryVariable;
