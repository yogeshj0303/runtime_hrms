...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="optionsDropdown">
                        ...
                        <li><a class="dropdown-item" href="{{ route('capture.deductionvariable.export-template') }}">Download Template</a></li>
                        <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#uploadExcelModal">Upload</a></li>
                        <li><a class="dropdown-item" href="{{ route('capture.deductionvariable.download-data') }}">Download Data</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card-body">
                <!-- Filters -->
                <form method="GET" action="{{ route('capture.deductionvariable') }}">
                    <div class="row mb-3">
                        <div class="col-md-2">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="salary_deduction_id" class="form-select form-select-sm" required>
                                <option value="">-- Select --</option>
                                @foreach($deductionComponents as $comp)
                                    <option value="{{ $comp->id }}" {{ request('salary_deduction_id') == $comp->id ? 'selected' : '' }}>{{ $comp->name }}</option>
                                @endforeach
                            </select>
                        </div>
...
<div class="modal fade" id="nonCashSalaryModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('capture.deductionvariable.transfer-non-cash') }}" method="POST">
            @csrf
...
<div class="modal fade" id="uploadExcelModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('capture.deductionvariable.upload-excel') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                <div class="modal-header border-bottom-0 pb-0">
...
<script>
        ...
        function saveEntry(empId, amount, comments, btn, successCallback) {
            var salaryCompId = $('select[name="salary_deduction_id"]').val();
            ...
            $.ajax({
                url: "{{ route('capture.deductionvariable.store') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    employee_id: empId,
                    salary_deduction_id: salaryCompId,
        ...
        function loadSalaryDetails(empId) {
            ...
            $.ajax({
                url: "{{ route('capture.deductionvariable.details') }}",
                type: "GET",
                data: {
                    employee_id: empId,
                    salary_deduction_id: salaryCompId,
        ...
        if (salaryDetailsModalEl) {
            salaryDetailsModalEl.addEventListener('show.bs.modal', function (event) {
                try {
                    ...
                    var salaryCompId = $('select[name="salary_deduction_id"]').val();
                    var salaryCompName = $('select[name="salary_deduction_id"] option:selected').text();
        ...
        $(document).off('click', '.delete-detail-btn').on('click', '.delete-detail-btn', function(e) {
            ...
            $.ajax({
                url: "{{ route('capture.deductionvariable.delete', '') }}/" + id,
                type: "DELETE",
                data: {
                    _token: "{{ csrf_token() }}"
                },
