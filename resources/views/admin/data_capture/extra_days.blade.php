...
@section('title') Extra Days @endsection

@section('content')
@component('components.breadcrumb')
    @slot('li_1') Data Capture @endslot
    @slot('title') Extra Days @endslot
@endcomponent
        ...
        <div class="card">
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    <h5 class="card-title mb-0 flex-grow-1">Capture extra days for which salary should be paid to employees.</h5>
            ...
            <div class="card-body border border-dashed border-end-0 border-start-0">
                <form method="GET" action="{{ route('capture.extradays') }}">
                    <div class="row g-3">
                        ...
                        <div class="col-xxl-3 col-sm-6">
                            <div>
                                ...
                                <div class="input-group">
                                    <input type="text" name="employee_search" class="form-control" placeholder="Search..." value="{{ request('employee_search') }}">
                        ...
                        <table class="table table-nowrap align-middle mb-0">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th scope="col" style="width: 100px;">EXTRA</th>
                                    <th scope="col" style="width: 100px;">ARREAR</th>
                                    <th scope="col" style="width: 100px;">OT</th>
                            ...
                            <tbody>
                                @forelse($employees as $index => $employee)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" class="form-control form-control-sm inline-input text-end" id="extra_{{ $employee->id }}" value="{{ $employee->current_extra_days }}" step="0.5">
                                        </td>
                                        <td>
                                            <input type="number" class="form-control form-control-sm inline-input text-end" id="arrear_{{ $employee->id }}" value="{{ $employee->current_arrear_days }}" step="0.5">
                                        </td>
                                        <td>
                                            <input type="number" class="form-control form-control-sm inline-input text-end" id="ot_{{ $employee->id }}" value="{{ $employee->current_ot_days }}" step="0.5">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control form-control-sm inline-input" id="comments_{{ $employee->id }}" value="{{ $employee->current_comments }}">
                                        </td>
                                        <td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="text-center text-muted">No employees found matching the criteria.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
<div class="modal fade" id="copyPreviousModal" tabindex="-1" aria-labelledby="copyPreviousModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('capture.extradays.copy') }}" method="POST">
            @csrf
            ...
            <div class="modal-content border-0">
                ...
                <div class="modal-body p-4">
                    <p class="text-muted mb-4">You are about to copy Extra Days, Arrear, OT, and Comments from a selected previous month into the current target month for all <strong class="text-primary">{{ isset($employees) ? count($employees) : 0 }}</strong> loaded employees.</p>
...
<script>
    $(document).ready(function() {
        // AJAX save Extra Days
        $(document).on('click', '.inline-save-btn', function(e) {
            e.preventDefault();
            
            var btn = $(this);
            var empId = btn.data('emp-id');
            var extra = $('#extra_' + empId).val();
            var arrear = $('#arrear_' + empId).val();
            var ot = $('#ot_' + empId).val();
            var comments = $('#comments_' + empId).val();
            ...
            $.ajax({
                url: '{{ route('capture.extradays.store') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    employee_id: empId,
