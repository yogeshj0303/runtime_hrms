...
@section('title') Extra Hours @endsection

@section('content')
@component('components.breadcrumb')
    @slot('li_1') Data Capture @endslot
    @slot('title') Extra Hours @endslot
@endcomponent
        ...
        <div class="card">
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    <h5 class="card-title mb-0 flex-grow-1">Capture extra hours to be paid to employees for time-based salary.</h5>
            ...
            <div class="card-body border border-dashed border-end-0 border-start-0">
                <form method="GET" action="{{ route('capture.extrahours') }}">
                    <div class="row g-3">
                            ...
                            <tbody>
                                @forelse($employees as $index => $employee)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="text-dark">{{ $employee->currentWorkProfile->department->name ?? 'N/A' }}</div>
                                        </td>
                                        <td>
                                            <input type="number" class="form-control form-control-sm inline-input text-end" id="hours_{{ $employee->id }}" value="{{ $employee->current_hours }}" step="0.5">
                                        </td>
                                        <td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center text-muted">No employees found matching the criteria.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
<div class="modal fade" id="copyPreviousModal" tabindex="-1" aria-labelledby="copyPreviousModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('capture.extrahours.copy') }}" method="POST">
            @csrf
            ...
            <div class="modal-content border-0">
                ...
                <div class="modal-body p-4">
                    <p class="text-muted mb-4">You are about to copy Extra Hours and Comments from a selected previous month into the current target month for all <strong class="text-primary">{{ isset($employees) ? count($employees) : 0 }}</strong> loaded employees.</p>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.inline-save-btn', function(e) {
            ...
            $.ajax({
                url: '{{ route('capture.extrahours.store') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    employee_id: empId,
