...
<div class="header-content">
    ...
    <div class="header-buttons">
        ...
        <a href="{{ route('locations.create') }}" class="btn-add">
...
<div class="row">
    ...
    <div class="col-md-4 mb-4">
        <div class="card h-100 shadow-sm border-0" style="background-color: {{ $location->is_default ? '#eef2fb' : '#fff' }}; border-radius: 12px;">
            <div class="card-body text-center p-4 d-flex flex-column">
                ...
                <div class="text-start mb-4">
                    ...
                    <div class="text-muted fs-13">{{ $location->work_profiles_count ?? 0 }} employee(s)</div>
                </div>

                <div class="d-flex justify-content-between text-start mb-4">
                    <div>
                        <div class="text-muted" style="font-size: 11px;">Location Head</div>
                        <div class="fw-medium text-dark" style="font-size: 13px;">{{ $location->site_head ?? 'Not Defined' }}</div>
                    </div>
                    <div class="text-end">
                        <div class="text-muted" style="font-size: 11px;">Deputy Head</div>
                        <div class="fw-medium text-dark" style="font-size: 13px;">{{ $location->deputy_head ?? 'Not Defined' }}</div>
                    </div>
                </div>

                <div class="d-flex justify-content-center mt-auto">
                    <div class="d-flex align-items-center bg-light rounded-pill px-3 py-2 shadow-sm" style="gap: 15px;">
                        <a href="{{ route('locations.edit', $location->id) }}" class="btn btn-sm btn-icon text-secondary p-0" title="Edit">
                            <i class="ri-pencil-fill fs-5"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-icon text-success p-0" title="Set Map Location" onclick="openMapModal({{ $location->id }}, '{{ $location->latitude }}', '{{ $location->longitude }}', '{{ addslashes($location->name) }}')">
                            <i class="ri-map-pin-user-fill fs-5"></i>
                        </button>
                        <form action="{{ route('locations.destroy', $location->id) }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Are you sure you want to delete this Location?');">
                            @csrf
                            @method('DELETE')
...
$('#saveLocationBtn').click(function() {
    ...
    let updateUrl = "{{ route('locations.update_map', ':id') }}".replace(':id', id);
