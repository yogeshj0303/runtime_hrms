...
@section('profile_title', 'Salary')
@section('profile_description', 'Manage employee salary, increment and revisions.')
...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('employee.statement', ['id' => $employee->id]) }}" class="btn-hrms-crimson"><i class="ri-file-download-line"></i> Download</a>
            <a href="{{ route('employee.profile.salary', ['id' => $employee->id, 'action' => 'add']) }}#salaryFormCard" class="btn-hrms-crimson"><i class="ri-add-circle-line"></i> Add Revision</a>
        </div>
    </div>
    <div class="card-body">
                ...
                <table class="table table-borderless align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="border-bottom">
                            <td class="fw-bold fs-14 text-dark">{{ strtoupper(\Carbon\Carbon::parse($revision->effective_from)->format('d-M-Y')) }}</td>
...
<div class="card shadow-sm border-0" id="salaryFormCard">
    ...
    @endphp
    <form action="{{ $editId ? route('employee.profile.salary.update', ['id' => $employee->id, 'revision_id' => $editId]) : route('employee.profile.salary.store', ['id' => $employee->id]) }}" method="POST">
        @csrf
        @if($editId) @method('PUT') @endif
        
        <div class="card-header bg-white border-bottom py-3">
            <h6 class="mb-3 fw-bold text-primary">{{ $editId ? 'Edit Salary Revision' : ($latestRev ? 'New Salary Revision (Cloned from Latest)' : 'New Salary Revision') }}</h6>
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center gap-4">
                    <div class="d-flex align-items-center">
                        <span class="fw-bold me-2">Effective From:</span>
                        <input type="month" name="effective_from" class="form-control form-control-sm" style="width: 150px;" required value="{{ ($editId && $latestRev) ? \Carbon\Carbon::parse($latestRev->effective_from)->format('Y-m') : date('Y-m') }}">
                    </div>
                <div class="d-flex align-items-center">
                    ...
                    <select name="salary_structure_id" id="salary_structure_id" class="form-select form-select-sm select2" style="width: 200px;" required>
                        ...
                        @foreach($salaryStructures as $structure)
                            <option value="{{ $structure->id }}" {{ ($latestRev && $latestRev->salary_structure_id == $structure->id) ? 'selected' : '' }}>
            ...
            <div class="row">
                ...
                <div class="col-md-7 border-end pe-4">
                    ...
                    <div id="dynamic-components-container">
                        @if($latestRev && $latestRev->custom_components)
                            @foreach($latestRev->custom_components as $comp_id => $amount)
                                <div class="row mb-3 align-items-center dynamic-component-row">
                                    ...
                                    <div class="col-sm-4">
                                        <input type="number" class="form-control text-end dynamic-amount-input" name="custom_components[{{ $comp_id }}]" value="{{ $amount }}" data-comp-id="{{ $comp_id }}">
                                    </div>
                                </div>
                            @endforeach
                        @else
                    ...
                    <input type="hidden" name="basic_salary" value="{{ $latestRev->basic_salary ?? 0 }}">
                    <input type="hidden" name="hra" value="{{ $latestRev->hra ?? 0 }}">
                    
                    <!-- Totals -->
                    <div class="row mb-3 align-items-center">
                        <label class="col-sm-8 col-form-label fw-bold text-dark fs-6">Gross Salary</label>
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end fw-bold" name="gross_salary" value="{{ $latestRev->gross_salary ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end fw-bold" name="total_salary" value="{{ $latestRev->total_salary ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="pt" value="{{ $latestRev->pt ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="esi" value="{{ $latestRev->esi ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="pf" value="{{ $latestRev->pf ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[lwf_deduction]" value="{{ $latestRev->contributions['lwf_deduction'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end fw-bold text-primary" name="net_salary" value="{{ $latestRev->net_salary ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[esi]" value="{{ $latestRev->contributions['esi'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[pf]" value="{{ $latestRev->contributions['pf'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[pension]" value="{{ $latestRev->contributions['pension'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[edli]" value="{{ $latestRev->contributions['edli'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end bg-light" name="contributions[lwf_contribution]" value="{{ $latestRev->contributions['lwf_contribution'] ?? 0 }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4 align-items-center bg-light p-2 rounded border">
                        ...
                        <div class="col-sm-4">
                            <input type="number" class="form-control text-end fw-bold" name="ctc" value="{{ $latestRev->ctc ?? 0 }}" readonly>
                        </div>
                    </div>
                ...
                <div class="col-md-5 ps-4">
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="is_increment" id="is_increment" value="1" {{ $latestRev && $latestRev->is_increment ? 'checked' : '' }}>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[esi_do_not_deduct]" id="esi_do_not_deduct" value="1" {{ isset($latestRev->salary_options['esi_do_not_deduct']) && $latestRev->salary_options['esi_do_not_deduct'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="esi_do_not_deduct">DO NOT Deduct ESI</label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[esi_above_ceiling]" id="esi_above_ceiling" value="1" {{ isset($latestRev->salary_options['esi_above_ceiling']) && $latestRev->salary_options['esi_above_ceiling'] ? 'checked' : '' }}>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[pf_do_not_deduct]" id="pf_do_not_deduct" value="1" {{ isset($latestRev->salary_options['pf_do_not_deduct']) && $latestRev->salary_options['pf_do_not_deduct'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="pf_do_not_deduct">DO NOT Deduct PF</label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[pf_no_pension]" id="pf_no_pension" value="1" {{ isset($latestRev->salary_options['pf_no_pension']) && $latestRev->salary_options['pf_no_pension'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="pf_no_pension">DO NOT Deduct Pension (PF)</label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[pf_above_ceiling_employee]" id="pf_above_ceiling_employee" value="1" {{ isset($latestRev->salary_options['pf_above_ceiling_employee']) && $latestRev->salary_options['pf_above_ceiling_employee'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="pf_above_ceiling_employee">Deduct PF Above Ceiling (Employee)</label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[pf_above_ceiling_employer]" id="pf_above_ceiling_employer" value="1" {{ isset($latestRev->salary_options['pf_above_ceiling_employer']) && $latestRev->salary_options['pf_above_ceiling_employer'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="pf_above_ceiling_employer">Deduct PF Above Ceiling (Employer)</label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[pf_on_gross]" id="pf_on_gross" value="1" {{ isset($latestRev->salary_options['pf_on_gross']) && $latestRev->salary_options['pf_on_gross'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="pf_on_gross">Deduct PF on Gross Salary</label>
                        </div>
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" name="options[pf_extra]" id="pf_extra" value="1" {{ isset($latestRev->salary_options['pf_extra']) && $latestRev->salary_options['pf_extra'] ? 'checked' : '' }}>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            ...
                            <input type="number" class="form-control form-control-sm text-end" style="width: 120px;" name="options[pf_min_deduction]" value="{{ $latestRev->salary_options['pf_min_deduction'] ?? 0 }}">
                        </div>
                    </div>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" name="options[pt_do_not_deduct]" id="pt_do_not_deduct" value="1" {{ isset($latestRev->salary_options['pt_do_not_deduct']) && $latestRev->salary_options['pt_do_not_deduct'] ? 'checked' : '' }}>
                                ...
                                <select name="options[pt_state]" class="form-select form-select-sm select2">
                                    ...
                                    @if(isset($statesData['states']))
                                        @foreach($statesData['states'] as $state)
                                            <option value="{{ $state['state'] }}" {{ (isset($latestRev->salary_options['pt_state']) && $latestRev->salary_options['pt_state'] == $state['state']) ? 'selected' : '' }}>{{ $state['state'] }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[tds_do_not_deduct]" id="tds_do_not_deduct" value="1" {{ isset($latestRev->salary_options['tds_do_not_deduct']) && $latestRev->salary_options['tds_do_not_deduct'] ? 'checked' : '' }}>
                            <label class="form-check-label" for="tds_do_not_deduct">DO NOT Deduct <i class="ri-information-line text-primary"></i></label>
                        </div>
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="options[tds_metro]" id="tds_metro" value="1" {{ isset($latestRev->salary_options['tds_metro']) && $latestRev->salary_options['tds_metro'] ? 'checked' : '' }}>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" name="options[lwf_do_not_deduct]" id="lwf_do_not_deduct" value="1" {{ isset($latestRev->salary_options['lwf_do_not_deduct']) && $latestRev->salary_options['lwf_do_not_deduct'] ? 'checked' : '' }}>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div style="width: 65%;">
                                <select name="options[lwf_state]" id="lwf_state_select" class="form-select form-select-sm select2">
                                    ...
                                    @if(isset($statesData['states']))
                                        @foreach($statesData['states'] as $state)
                                            <option value="{{ $state['state'] }}" {{ (isset($latestRev->salary_options['lwf_state']) && $latestRev->salary_options['lwf_state'] == $state['state']) ? 'selected' : '' }}>{{ $state['state'] }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const epfSetting = @json($epfSetting);
    const epfComponentIds = @json($epfComponentIds ?? []);
    const esiSetting = @json($esiSetting);
    const esiComponentIds = @json($esiComponentIds ?? []);
    const ptaxSetting = @json($ptaxSetting);
    const ptaxSlabs = @json($ptaxSlabs ?? []);
    const ptaxComponentIds = @json($ptaxComponentIds ?? []);
    const lwfRules = @json($lwfRules ?? []);
    ...
    if (editStructureId) {
        fetch(`{{ url('business/employee/profile/salary/structure') }}/${editStructureId}`)
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
