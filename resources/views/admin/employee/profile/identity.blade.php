...
@section('profile_title', 'Identity')
@section('profile_description', 'Manage Identity records and official government verifications.')
...
<div class="row">
    ...
    <div class="col-lg-8 pe-lg-4 border-end">
        ...
        <div class="mb-4 pb-3 border-bottom">
            ...
            <form action="{{ route('employee.profile.identity.update') }}" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ $employee->id }}">
                <div class="row mb-3 align-items-center">
                    <div class="col-md-4"><label class="mb-0 fs-13 text-muted">Bank Name</label></div>
                    <div class="col-md-8">
                        <input type="text" name="bank_name" class="form-control form-control-sm" value="{{ $identity->bank_name ?? '' }}" placeholder="e.g. HDFC Bank">
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    <div class="col-md-4"><label class="mb-0 fs-13 text-muted">IFSC Code</label></div>
                    <div class="col-md-8">
                        <input type="text" name="ifsc" class="form-control form-control-sm text-uppercase" value="{{ $identity->ifsc ?? '' }}" placeholder="HDFC0001234">
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    <div class="col-md-4"><label class="mb-0 fs-13 text-muted">Account Number</label></div>
                    <div class="col-md-8">
                        <input type="text" name="account_number" class="form-control form-control-sm" value="{{ $identity->account_number ?? '' }}" placeholder="01234567890">
                    </div>
                </div>
                <div class="d-flex gap-2 mt-3">
                    <button type="submit" class="btn-hrms-crimson"><i class="ri-save-line"></i> Save</button>
                    @if(isset($identity) && $identity->is_bank_verified)
        ...
        <div class="mb-4 pb-3 border-bottom">
            ...
            <form action="{{ route('employee.profile.identity.update') }}" method="POST">
                ...
                <div class="row mb-3 align-items-center">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="aadhaar_number" class="form-control form-control-sm" maxlength="12" value="{{ $identity->aadhaar_number ?? '' }}" placeholder="1234 5678 9012">
                    </div>
                </div>
                <div class="d-flex gap-2 mt-3">
                    <button type="submit" class="btn-hrms-crimson"><i class="ri-save-line"></i> Save</button>
                    @if(isset($identity) && $identity->is_aadhaar_verified)
                        <span class="badge bg-success-subtle text-success border border-success d-inline-flex align-items-center px-2 py-1"><i class="ri-checkbox-circle-line me-1"></i> Verified</span>
                    @else
        ...
        <div class="mb-4 pb-3 border-bottom">
            ...
            <form action="{{ route('employee.profile.identity.update') }}" method="POST">
                ...
                <div class="row mb-3 align-items-center">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="pan_number" class="form-control form-control-sm text-uppercase" maxlength="10" value="{{ $identity->pan_number ?? '' }}" placeholder="ABCDE1234F">
                    </div>
                </div>
                <div class="d-flex gap-2 mt-3">
                    <button type="submit" class="btn-hrms-crimson"><i class="ri-save-line"></i> Save</button>
                    @if(isset($identity) && $identity->is_pan_verified)
                        <span class="badge bg-success-subtle text-success border border-success d-inline-flex align-items-center px-2 py-1"><i class="ri-checkbox-circle-line me-1"></i> Verified</span>
                    @else
        ...
        <div class="mb-2">
            ...
            <form action="{{ route('employee.profile.identity.update') }}" method="POST">
                ...
                <div class="row mb-3 align-items-center">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="passport" class="form-control form-control-sm" value="{{ $identity->passport ?? '' }}">
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    <div class="col-md-4"><label class="mb-0 fs-13 text-muted">Driving License #</label></div>
                    <div class="col-md-8">
                        <input type="text" name="driving_license" class="form-control form-control-sm" value="{{ $identity->driving_license ?? '' }}">
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    ...
                    <div class="col-md-8">
                        <select name="blood_group" class="form-select form-select-sm">
                            ...
                            @foreach(['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'] as $bg)
                                <option value="{{ $bg }}" {{ isset($identity) && $identity->blood_group == $bg ? 'selected' : '' }}>{{ $bg }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="pf_uan" class="form-control form-control-sm" value="{{ $identity->pf_uan ?? '' }}">
                    </div>
                </div>
                <div class="row mb-3 align-items-center">
                    <div class="col-md-4"><label class="mb-0 fs-13 text-muted">ESI IP Number</label></div>
                    <div class="col-md-8">
                        <input type="text" name="esi_number" class="form-control form-control-sm" value="{{ $identity->esi_number ?? '' }}">
                    </div>
                </div>
                <div class="d-flex gap-2 mt-3">
    ...
    <div class="col-lg-4 ps-lg-4 mt-4 mt-lg-0">
        <div class="p-3 bg-light rounded border">
            ...
            <form action="{{ route('employee.profile.identity.update') }}" method="POST" id="kycForm">
                @csrf
                <input type="hidden" name="id" value="{{ $employee->id }}">
                <input type="hidden" name="is_kyc_done_submit" value="1">
                <div class="form-check form-switch mb-2">
                    <input class="form-check-input" type="checkbox" role="switch" name="is_kyc_done" id="is_kyc_done" value="1" {{ isset($identity) && $identity->is_kyc_done ? 'checked' : '' }} onchange="document.getElementById('kycForm').submit();">
                    <label class="form-check-label fs-13 fw-semibold" for="is_kyc_done">KYC Completed</label>
                </div>
            </form>
            @if(isset($identity) && $identity->is_kyc_done)
