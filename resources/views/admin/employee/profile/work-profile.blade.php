...
@section('profile_title', 'Work Profile')
@section('profile_description', "Specify employee's profile within organization.")
            ...
            @if($currentWork)
                ...
                <div class="mb-4">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <select class="form-select form-select-sm" style="max-width: 160px; font-weight: 600;" onchange="location.href='?id={{ $employee->id }}&revision_id=' + this.value">
                            @foreach($employee->workProfiles as $wp)
                                <option value="{{ $wp->id }}" {{ $wp->id == $currentWork->id ? 'selected' : '' }}>
                                    {{ \Carbon\Carbon::parse($wp->effective_from)->format('M-Y') }} {{ $wp->is_current ? '(Current)' : '' }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="work-meta-row">
                    ...
                    <div class="work-meta-value text-uppercase">
                        {{ \App\Models\BusinessUnit::find($currentWork->business_unit_id)?->unit_name ?? \App\Models\BusinessUnit::find($currentWork->business_unit_id)?->business_name ?? 'SOMYA AUTOCAR PRIVATE LIMITED' }}
                    </div>
                </div>
                ...
                <div class="work-meta-row">
                    ...
                    <div class="work-meta-value text-uppercase" style="color: #1e293b;">
                        {{ \App\Models\Location::find($currentWork->location_id)?->name ?? 'INDORE WORKSHOP JAWA' }}
                    </div>
                </div>
                ...
                <div class="work-meta-row">
                    ...
                    <div class="work-meta-value">
                        {{ \App\Models\CostCenter::find($currentWork->cost_center_id)?->name ?? 'General Cost Center' }}
                    </div>
                </div>
                ...
                <div class="work-meta-row">
                    ...
                    <div class="work-meta-value">
                        {{ \App\Models\Department::find($currentWork->department_id)?->name ?? 'Service' }}
                    </div>
                </div>
                ...
                <div class="work-meta-row">
                    ...
                    <div class="work-meta-value">
                        {{ \App\Models\Grade::find($currentWork->grade_id)?->name ?? 'Executive' }}
                    </div>
                </div>
                ...
                <div class="work-meta-row mb-0">
                    ...
                    <div class="work-meta-value">
                        {{ \App\Models\Designation::find($currentWork->designation_id)?->name ?? 'CRE' }}
                    </div>
                </div>
            @else
                    ...
                    <div class="manager-item-left">
                        @if($repManager)
                            <div class="manager-item-avatar">
                                {{ strtoupper(substr($repManager->first_name ?? 'N', 0, 1) . substr($repManager->last_name ?? 'B', 0, 1)) }}
                            </div>
                            <div>
                                <div class="manager-item-name">{{ strtoupper($repManager->first_name . ' ' . $repManager->last_name) }}</div>
                                <div class="manager-item-role">{{ $repManager->designation ?? 'SERVICE MANAGER' }}</div>
                            </div>
                        @else
            ...
            <div class="mb-4">
                ...
                <div class="manager-item-box">
                    <div class="manager-item-left">
                        @if($hrManager)
                            <div class="manager-item-avatar">
                                {{ strtoupper(substr($hrManager->first_name ?? 'N', 0, 1) . substr($hrManager->last_name ?? 'B', 0, 1)) }}
                            </div>
                            <div>
                                <div class="manager-item-name">{{ strtoupper($hrManager->first_name . ' ' . $hrManager->last_name) }}</div>
                                <div class="manager-item-role">{{ $hrManager->designation ?? 'SERVICE MANAGER' }}</div>
                            </div>
                        @else
                            <div class="manager-item-avatar">NB</div>
                            <div>
            ...
            <div>
                ...
                <div class="manager-item-box">
                    <div class="manager-item-left">
                        @if($indirectManager)
                            <div class="manager-item-avatar">
                                {{ strtoupper(substr($indirectManager->first_name ?? 'I', 0, 1) . substr($indirectManager->last_name ?? 'M', 0, 1)) }}
                            </div>
                            <div>
                                <div class="manager-item-name">{{ strtoupper($indirectManager->first_name . ' ' . $indirectManager->last_name) }}</div>
                                <div class="manager-item-role">{{ $indirectManager->designation ?? 'Manager' }}</div>
                            </div>
                        @else
...
<div class="modal fade" id="workProfileModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="workProfileForm" method="POST" action="{{ route('employee.profile.work-profile.store', ['id' => $employee->id]) }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="month" class="form-control" name="effective_from" id="effective_from" required value="{{ date('Y-m') }}">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="business_unit_id" id="business_unit_id" required>
                                ...
                                @foreach($businessUnits as $bu)
                                    <option value="{{ $bu->id }}">{{ $bu->unit_name ?? $bu->business_name ?? 'Unit ' . $bu->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="location_id" id="location_id" required>
                                ...
                                @foreach($locations as $loc)
                                    <option value="{{ $loc->id }}">{{ $loc->name ?? 'Loc ' . $loc->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="cost_center_id" id="cost_center_id">
                                ...
                                @foreach($costCenters as $cc)
                                    <option value="{{ $cc->id }}">{{ $cc->name ?? 'CC ' . $cc->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="department_id" id="department_id" required>
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->id }}">{{ $dept->name ?? 'Dept ' . $dept->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="grade_id" id="grade_id">
                                ...
                                @foreach($grades as $gr)
                                    <option value="{{ $gr->id }}">{{ $gr->name ?? 'Grade ' . $gr->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="designation_id" id="designation_id" required>
                                ...
                                @foreach($designations as $des)
                                    <option value="{{ $des->id }}">{{ $des->name ?? 'Desig ' . $des->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="reporting_manager_id" id="reporting_manager_id">
                                ...
                                @foreach($managers as $m)
                                    <option value="{{ $m->id }}">{{ $m->first_name }} {{ $m->last_name }} ({{ $m->employee_code }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
...
<div class="modal fade" id="assignManagerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ $currentWork ? route('employee.profile.work-profile.update', ['id' => $employee->id, 'work_profile_id' => $currentWork->id]) : route('employee.profile.work-profile.store', ['id' => $employee->id]) }}">
                @csrf
                @if($currentWork)
                    <input type="hidden" name="effective_from" value="{{ \Carbon\Carbon::parse($currentWork->effective_from)->format('Y-m') }}">
                    <input type="hidden" name="business_unit_id" value="{{ $currentWork->business_unit_id }}">
                    <input type="hidden" name="location_id" value="{{ $currentWork->location_id }}">
                    <input type="hidden" name="cost_center_id" value="{{ $currentWork->cost_center_id }}">
                    <input type="hidden" name="department_id" value="{{ $currentWork->department_id }}">
                    <input type="hidden" name="grade_id" value="{{ $currentWork->grade_id }}">
                    <input type="hidden" name="designation_id" value="{{ $currentWork->designation_id }}">
                    <input type="hidden" name="reporting_manager_id" id="modal_rep_id" value="{{ $currentWork->reporting_manager_id }}">
                    <input type="hidden" name="hr_manager_id" id="modal_hr_id" value="{{ $currentWork->hr_manager_id }}">
                    <input type="hidden" name="indirect_manager_id" id="modal_ind_id" value="{{ $currentWork->indirect_manager_id }}">
                @endif
                <div class="modal-header">
...
<script>
    ...
    function openAddModal() {
        ...
        document.getElementById('workProfileForm').action = "{{ route('employee.profile.work-profile.store', ['id' => $employee->id]) }}";
    ...
    function openEditModal(id, effectiveFrom, buId, locId, ccId, deptId, grId, desId, repId, hrId, indId, isPromo) {
        ...
        document.getElementById('workProfileForm').action = "/employee/profile/work-profile/" + id + "?id={{ $employee->id }}";
