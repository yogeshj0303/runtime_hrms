...
@section('profile_title', 'Basic Info')
@section('profile_description', "Manage core HR related employee information.")

@section('profile_actions')
<a href="{{ route('employee.statement', ['id' => $employee->id]) }}" class="btn-hrms-crimson me-2">
...
<form action="{{ route('employee.profile.basic.update', ['id' => $employee->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-md-3 text-center">
            <div class="position-relative d-inline-block mb-2" onclick="document.getElementById('profile_photo').click()" style="cursor: pointer;">
                @if($employee->profile && $employee->profile->profile_photo)
                    <img src="{{ asset('storage/' . $employee->profile->profile_photo) }}" class="rounded-circle" style="width: 86px; height: 86px; object-fit: cover;">
                @else
                    <div class="rounded-circle d-flex justify-content-center align-items-center" style="width: 86px; height: 86px; font-size: 32px; font-weight: 700; background-color: #bde7f4; color: #0e3d50; margin: 0 auto;">
                        {{ strtoupper(substr($employee->first_name ?? 'A', 0, 1) . substr($employee->last_name ?? 'Y', 0, 1)) }}
                    </div>
                @endif
            </div>
        ...
        <div class="col-md-9">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-text" style="font-size: 11.5px;">Name as per Bank <br><span class="text-dark fw-bold">{{ strtoupper($employee->first_name . ' ' . $employee->last_name) }}</span></div>
                </div>
                <div class="col-md-4">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="date" class="form-control" name="joining_date" value="{{ $employee->profile?->joining_date ?? $employee->joining_date ? \Carbon\Carbon::parse($employee->profile?->joining_date ?? $employee->joining_date)->format('Y-m-d') : '' }}" required>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="date" class="form-control" name="confirmation_date" value="{{ $employee->profile?->confirmation_date ?? '' }}">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="mobile_number" value="{{ $employee->phone }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="office_phone" value="{{ $employee->profile?->office_phone ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="email" class="form-control" name="official_email" value="{{ $employee->profile?->official_email ?? $employee->email }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="biometric_code" value="{{ $employee->profile?->biometric_code ?? $employee->employee_code }}">
                </div>
            </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-sm-4 d-flex align-items-center">
                    <input type="number" class="form-control me-2" name="notice_period" value="{{ $employee->profile?->notice_period ?? 0 }}">
                    <span>days</span>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="date" class="form-control" name="dob" value="{{ $employee->profile?->dob ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8 pt-2">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" id="genderMale" value="Male" {{ ($employee->profile?->gender ?? '') == 'Male' ? 'checked' : '' }}>
                        <label class="form-check-label" for="genderMale">Male</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" id="genderFemale" value="Female" {{ ($employee->profile?->gender ?? '') == 'Female' ? 'checked' : '' }}>
                        <label class="form-check-label" for="genderFemale">Female</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" id="genderTransgender" value="Transgender" {{ ($employee->profile?->gender ?? '') == 'Transgender' ? 'checked' : '' }}>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8 pt-2">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="marital_status" id="maritalUnmarried" value="Unmarried" {{ ($employee->profile?->marital_status ?? '') == 'Unmarried' ? 'checked' : '' }}>
                        <label class="form-check-label" for="maritalUnmarried">Unmarried</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="marital_status" id="maritalMarried" value="Married" {{ ($employee->profile?->marital_status ?? '') == 'Married' ? 'checked' : '' }}>
                        <label class="form-check-label" for="maritalMarried">Married</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="marital_status" id="maritalWidow" value="Widow" {{ ($employee->profile?->marital_status ?? '') == 'Widow' ? 'checked' : '' }}>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="email" class="form-control" name="personal_email" value="{{ $employee->profile?->personal_email ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="personal_phone" value="{{ $employee->profile?->personal_phone ?? '' }}">
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="emergency_contact" value="{{ $employee->profile?->emergency_contact ?? '' }}">
                </div>
            </div>
            ...
            <div class="card mb-4 shadow-sm">
                <div class="card-body text-center">
                    ...
                    @if($employee->face_image)
                        <img src="{{ $employee->face_image }}" class="img-fluid rounded mb-3" style="max-height: 200px; object-fit: cover; border: 2px solid #ddd;">
                    @else
                        ...
                        @else
                            @foreach($tags as $tag)
                                <span class="badge bg-primary me-1 mb-1 tag-badge" style="font-size: 14px;">
                                    {{ $tag }} 
                                    <i class="ri-close-line ms-1" style="cursor: pointer;" onclick="removeTag('{{ $tag }}')"></i>
                                </span>
                            @endforeach
                        @endif
                    </div>
...
<script>
    ...
    btnSaveFace.addEventListener('click', function() {
        ...
        fetch("{{ route('employee.profile.face.update', ['id' => $employee->id]) }}", {
            method: 'POST',
            headers: {
    ...
    btnSaveTag.addEventListener('click', function() {
        ...
        fetch("{{ route('employee.profile.tags.add', ['id' => $employee->id]) }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
