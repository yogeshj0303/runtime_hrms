...
@section('profile_title', 'Summary')
@section('profile_description', "Summary of employee's record (read-only mode).")
        ...
        <div class="summary-left-card">
            ...
            <div class="d-flex align-items-center gap-4 mb-4">
                <div class="summary-avatar">
                    @if($employee->profile && $employee->profile->profile_photo)
                        <img src="{{ asset('storage/' . $employee->profile->profile_photo) }}" alt="{{ $employee->first_name }}">
                    @else
                        {{ strtoupper(substr($employee->first_name ?? 'A', 0, 1) . substr($employee->last_name ?? 'Y', 0, 1)) }}
                    @endif
                </div>
                <div>
                    <h5 class="summary-emp-name">{{ $employee->first_name }} {{ $employee->last_name }}</h5>
                    <div class="summary-emp-code">{{ $employee->employee_code ?? '812' }}</div>
            ...
            <div class="summary-contact-grid mb-3">
                <div class="summary-contact-item">
                    ...
                    <span class="{{ $officialEmail !== 'Not added' ? 'summary-contact-email' : 'text-muted' }}">{{ $officialEmail }}</span>
                    @if($officialEmail !== 'Not added')
                        <button class="btn-copy-mini" title="Copy Email" onclick="copyToClipboard('{{ $officialEmail }}', this)">
                ...
                <div class="summary-contact-item">
                    ...
                    <span class="summary-contact-phone">{{ $phoneDisplay }}</span>
                    @if($phoneDisplay !== 'Not added')
                        <button class="btn-copy-mini" title="Copy Phone" onclick="copyToClipboard('{{ $phoneDisplay }}', this)">
                            <i class="ri-file-copy-line"></i>
                        </button>
                    @endif
                </div>
            ...
            <div class="mb-3">
                ...
                @if($repManager)
                    ...
                    <div class="manager-box-card">
                        <div class="manager-avatar-mini">{{ $repInitials }}</div>
                        <div>
                            <div>
                                <a href="{{ route('employee.profile.summary', ['id' => $repManager->id]) }}" class="manager-name-link">
                                    {{ strtoupper($repManager->first_name . ' ' . $repManager->last_name) }}
                                    <i class="ri-external-link-line"></i>
                                </a>
                            </div>
                            <div class="manager-code-subtitle">{{ $repManager->employee_code ?? '804' }}</div>
                        </div>
                    </div>
                @else
                    <div class="manager-box-card">
            ...
            <div class="mb-3">
                ...
                @if($hrManager)
                    ...
                    @endphp
                    <div class="manager-box-card">
                        <div class="manager-avatar-mini">{{ $hrInitials }}</div>
                        <div>
                            <div>
                                <a href="{{ route('employee.profile.summary', ['id' => $hrManager->id]) }}" class="manager-name-link">
                                    {{ strtoupper($hrManager->first_name . ' ' . $hrManager->last_name) }}
                                    <i class="ri-external-link-line"></i>
                                </a>
                            </div>
                            <div class="manager-code-subtitle">{{ $hrManager->employee_code ?? '804' }}</div>
                        </div>
                    </div>
                @else
                    <div class="manager-box-card">
            ...
            <div>
                ...
                @if($indirectManager)
                    ...
                    @endphp
                    <div class="manager-box-card">
                        <div class="manager-avatar-mini">{{ $indInitials }}</div>
                        <div>
                            <div>
                                <a href="{{ route('employee.profile.summary', ['id' => $indirectManager->id]) }}" class="manager-name-link">
                                    {{ strtoupper($indirectManager->first_name . ' ' . $indirectManager->last_name) }}
                                    <i class="ri-external-link-line"></i>
                                </a>
                            </div>
                            <div class="manager-code-subtitle">{{ $indirectManager->employee_code }}</div>
                        </div>
                    </div>
                @else
    ...
    <div class="col-lg-5 col-md-12">
        <div class="summary-right-card">
            ...
            <div class="mb-5">
                ...
                @if(isset($directReports) && $directReports->count() > 0)
                    @foreach($directReports as $report)
                        @if($report->employee)
                            <a href="{{ route('employee.profile.summary', ['id' => $report->employee->id]) }}" class="reportee-item-card">
                                <div class="manager-avatar-mini" style="width: 32px; height: 32px; font-size: 11px;">
                                    {{ strtoupper(substr($report->employee->first_name, 0, 1) . substr($report->employee->last_name ?? '', 0, 1)) }}
                                </div>
                                <div>
                                    <div class="fw-bold text-dark" style="font-size: 13px;">{{ $report->employee->first_name }} {{ $report->employee->last_name }}</div>
                                    <div class="text-muted" style="font-size: 11.5px;">{{ $report->designation ? $report->designation->name : ($report->employee->employee_code ?? '') }}</div>
                                </div>
                            </a>
                        @endif
                    @endforeach
            ...
            <div class="mb-5">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-6">
                        ...
                        <div class="summary-field-value">
                            <span>{{ $employee->profile?->notice_period ?? 0 }} day(s)</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row g-4">
                    <div class="col-6">
                        ...
                        <div class="summary-field-value val-muted">
                            {{ $employee->profile?->office_phone ?? 'Not Added' }}
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="summary-field-label">Emergency Contact</div>
                        <div class="summary-field-value val-muted">
                            {{ $employee->profile?->emergency_contact ?? 'Not Added' }}
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="summary-field-label">Home Phone</div>
                        <div class="summary-field-value val-muted">
                            {{ $employee->profile?->personal_phone ?? 'Not Added' }}
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="summary-field-label">Personal E-Mail</div>
                        <div class="summary-field-value val-muted">
                            {{ $employee->profile?->personal_email ?? 'Not Added' }}
                        </div>
                    </div>
                </div>
            </div>
